<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(!$nameErrors && !$desErrors && !$priceErrors) {
            $conn =  new Database();
            $pdo = $conn->getConnect();
            if (isset($_POST['selectedCategories']) && is_array($_POST['selectedCategories'])) {
                foreach ($_POST['selectedCategories'] as $selectedCategory) {
                    $dat = Product::addOneProduct($pdo, $name, $price, $des, $image, $selectedCategory);
                }
            }
